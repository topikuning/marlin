...
class MasterWorkCode(Base):
    """Kode standar pekerjaan lintas semua proyek. Kunci normalisasi BOQ."""
...
class BOQItem(Base):
    """Item pekerjaan dalam BOQ. Dirancang fleksibel untuk berbagai format."""
...
class WeeklyProgressItem(Base):
    """Progress aktual per item BOQ dalam satu minggu. Granularity tertinggi."""
...
class EarlyWarning(Base):
    ...
    # deviation | spi | time_work_ratio | no_report | item_stuck | end_sprint
...
class BOQImportLog(Base):
    """Audit trail setiap kali BOQ di-import dari Excel ke suatu fasilitas."""
    ...
    import_type       = Column(String(20), nullable=False)  # initial | addendum
    ...
    addendum = relationship("ContractAddendum")
    importer = relationship("User")

    __table_args__ = (
        Index("idx_boq_import_facility", "facility_id"),
    )
...
class BOQExcelTemplate(Base):
    """Template mapping kolom Excel untuk berbagai format BOQ per kontraktor/daerah."""
